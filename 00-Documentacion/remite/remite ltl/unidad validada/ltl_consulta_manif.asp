...
if Request.Form ("etapa") = "2" then
	...
	Response.Redirect "ltl_consulta_manif" & qa & ".asp?msg=" & Server.URLEncode("Manifiesto cancelado.") 
end if

Response.Write print_headers("Consulta Manifiestos", "ltl", "", "", "")
%>	
...
SQL = SQL &  " , TRA.TRACONS_GENERAL " & VbCrlf
SQL = SQL &  " , EAL.ALLCODIGO  " & VbCrlf
SQL = SQL &  " ORDER BY WEL_MANIF_NUM DESC " & VbCrlf
...
//<!--
	function desactivar_manif(wel_manif_num) {
...
<form name="desactivar_manif" id="desactivar_manif" action="ltl_consulta_manif<%=qa%>.asp" method="post">
	<input type="hidden" name="etapa" value="2">
	<input type="hidden" name="wel_manif_num" value="">
</form>

<script language="javascript">
function print_etiq(id, traclave) {
...
<form name="wel_manif_num" action="ltl_consulta<%=qa%>.asp?tipo=1" method="post">
	<input type="hidden" name="wel_manif_num" value="">
</form>

<script language="javascript">
function modif_manif(wel_manif_num, cliclef) {
...
<!--<form name="modif_manif" action="ltl_captura_encabezado<%=qa%>.asp" method="post">-->
<!-- PCLP -->
<form name="modif_manif" action="documentacion_nuis<%=qa%>.asp" method="post">
<!-- PCLP -->
	<input type="hidden" name="wel_manif_num" value="">
...
<table class="datos" align="center" BORDER="1" cellpadding="2" cellspacing="0" width="900">
  ...
  <tbody>
	...
	%>
    <tr align=center>
      <td>&nbsp;<a href="javascript:select_wel_manif_num('<%=arrayManif(0,iRowLoop)%>');" onmouseover="return overlib('Ver LTLs del Manifiesto');" onmouseout="return nd();"><%=arrayManif(0,iRowLoop)%></a></td>     
      ...
      if Session("ltl_doc_convertidor") = "1" then
        ...
        if arrayTmp(0, 0) = "0" then
			...
			'<<< CHG-DESA-14062024: Se cambia el RDF que imprime el Manifiesto:
				'tipo_Reporte = "OPER6304_web_ltl_manifiesto" & qa & ".rdf"
				tipo_Reporte = "OPER6304_web_ltl_manifiesto" & test & ".rdf"
			'    CHG-DESA-14062024 >>>
        end if
      else
		'<<< CHG-DESA-14062024: Se cambia el RDF que imprime el Manifiesto:
			'tipo_Reporte = "OPER6304_web_ltl_manifiesto" & qa & ".rdf"
			'tipo_Reporte = "OPER6304_web_ltl_manifiesto" & test & ".rdf"
			tipo_Reporte = "OPER6304_web_ltl_manifiesto.rdf"
		'    CHG-DESA-14062024 >>>
      end if%>
      ...
      <td>&nbsp;
        ...
        arrayTmp = GetArrayRS(SQL)
        if arrayTmp(0,0) > "0" then%>
            <a href="javascript:modif_manif(<%=arrayManif(0,iRowLoop)%>, <%=arrayManif(1,iRowLoop)%>)"><img src="./images/accessories-text-editor.png" style="border:none; cursor:pointer; width: 16px; height: 16px" onmouseover="return overlib('Agregar talones.');" onmouseout="return nd();"></a> 
        <%end if%>
       &nbsp;&nbsp;&nbsp;<a href="/cgi/rwcgi60.exe/run?db_logis+wel_manif_num=<%=arrayManif(0,iRowLoop)%>+mi_cliclef=<%=arrayManif(1,iRowLoop)%>+report=<%=tipo_reporte%>+destype=cache+desformat=pdf"><img src="./images/document-print.png" style="border:none;rcursor:pointer; width: 16px; height: 16px;" onmouseover="return overlib('Imprimir el Manifiesto');" onmouseout="return nd();"></a> 
       <%if NVL(arrayManif(7,iRowLoop))="" then%> 
        &nbsp;&nbsp;&nbsp;<a href="javascript:desactivar_manif(<%=arrayManif(0,iRowLoop)%>);"><img src="./images/edit-delete.png" style="border:none; cursor:pointer; width: 16px; height: 16px;" onmouseover="return overlib('Cancelar el Manifiesto');" onmouseout="return nd();" alt="Cancelar manifiesto"></a> 
      <%end if%>
        ...
        &nbsp;&nbsp;&nbsp;<a href="javascript:print_etiq(<%=arrayManif(0,iRowLoop)%>, '<%=arrayManif(10,iRowLoop)%>');"><img src="./images/label.gif" style="border:none; cursor:pointer;" onmouseover="return overlib('Imprimir las etiquetas.');" onmouseout="return nd();" alt="Imprimir las etiquetas."></a> 
        &nbsp;&nbsp;&nbsp;<a href="ltl_print_talon.asp?manif_num=<%=arrayManif(0,iRowLoop)%>&cliclef=<%=arrayManif(1,iRowLoop)%>&tipo=manif"><img src="./images/document-print.png" style="border:none;rcursor:pointer; width: 16px; height: 16px;" onmouseover="return overlib('Imprimir los talones');" onmouseout="return nd();"></a> 
      </td>
    </tr>
    <%
	next
