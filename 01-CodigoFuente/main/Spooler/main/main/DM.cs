...
internal class DM
{
    ...
    public string trading_genera_GSK(string cliente, string? Fecha_1, string? Fecha_2, string? empresa, Int32? idCron, int? vs)
    {
        string SQL_GSK = " SELECT  \n"
            ...
            + "  AND TRACLAVE(+) = WCD.WCD_TRACLAVE AND TRASTATUS (+) = '1' AND TAE_TRACLAVE(+) = WCD.WCD_TRACLAVE\n"
                     ...
                     + "  UNION\n"
                     + "  SELECT\n"
                     + "    NVL(TDCD.TDCDFACTURA, WEL.WELFACTURA), '', '',\n"
                     + "    TO_CHAR(WEL.DATE_CREATED, 'DD/MM/YY'), '', INITCAP(DIS.DISNOM) REMITENTE,\n"
                     + "    INITCAP(DISADRESSE1 || ' ' || ' ' || DISNUMEXT || '  ' || DISNUMINT || '  ' ||DISADRESSE2 || DECODE(DISCODEPOSTAL,NULL,NULL, ' C.P. ' || DISCODEPOSTAL)),\n"
                     + "    INITCAP(CIU_ORI.VILNOM || ' ('|| EST_ORI.ESTNOMBRE || ')'),\n"
                     + "    INITCAP(CCL.CCL_NOMBRE || ' ' || NVL(DIE.DIE_A_ATENCION_DE, DIE.DIENOMBRE)),\n"
                     + "    INITCAP( DIEADRESSE1|| ' ' || ' ' || DIENUMEXT || '  ' || DIENUMINT || '  ' ||DIEADRESSE2 || DECODE(DIECODEPOSTAL,NULL,NULL, ' C.P. ' || DIECODEPOSTAL)),\n"
                     + "    INITCAP(CIU_DEST.VILNOM || ' ('|| EST_DEST.ESTNOMBRE || ')'),\n"
                     + "    'ROAD', WEL.WEL_FIRMA, TO_CHAR(WEL.DATE_CREATED, 'DD/MM/YY')\n"
                     + "  FROM\n"
                     + "    WEB_LTL WEL,\n"
